import 'dart:io';

import 'package:flutter/services.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:track_my_stuff/features/items/data/ml/mock_object_detector.dart';

void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  // Stub path_provider to return the system temp dir
  setUp(() {
    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
        .setMockMethodCallHandler(
      const MethodChannel('plugins.flutter.io/path_provider'),
      (call) async {
        if (call.method == 'getTemporaryDirectory') {
          return Directory.systemTemp.path;
        }
        return null;
      },
    );
  });

  tearDown(() {
    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
        .setMockMethodCallHandler(
      const MethodChannel('plugins.flutter.io/path_provider'),
      null,
    );
  });

  // Unit tests run on Linux where ML Kit is unavailable, so we test MockObjectDetector.
  group('MockObjectDetector (non-Android/iOS environment)', () {
    test('returns one DetectedObject covering the whole image', () async {
      final imageFile = await _writeTempJpeg();

      final detector = MockObjectDetector();
      final results = await detector.detectObjects(imageFile);

      expect(results.length, 1);
      expect(results.first.label, 'Object');
      expect(results.first.confidence, 1.0);
      expect(results.first.imageFile.existsSync(), isTrue);

      imageFile.deleteSync();
      results.first.imageFile.deleteSync();
    });

    test('bounding box covers full image dimensions', () async {
      final imageFile = await _writeTempJpeg();

      final detector = MockObjectDetector();
      final results = await detector.detectObjects(imageFile);

      expect(results.first.boundingBox.left, 0);
      expect(results.first.boundingBox.top, 0);
      expect(results.first.boundingBox.width, greaterThan(0));
      expect(results.first.boundingBox.height, greaterThan(0));

      imageFile.deleteSync();
      results.first.imageFile.deleteSync();
    });

    test('returns empty list for undecodable bytes', () async {
      final badFile = File('${Directory.systemTemp.path}/bad_image.jpg');
      await badFile.writeAsBytes([0, 1, 2, 3]);

      final detector = MockObjectDetector();
      final results = await detector.detectObjects(badFile);

      expect(results, isEmpty);
      badFile.deleteSync();
    });
  });
}

/// Writes a minimal valid 1×1 JPEG to a temp file and returns it.
Future<File> _writeTempJpeg() async {
  final bytes = [
    ,
  ];
  final file = File('${Directory.systemTemp.path}/test_image.jpg');
  await file.writeAsBytes(bytes);
  return file;
}
